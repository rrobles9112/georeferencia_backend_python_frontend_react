...
async def concentracion_handler(request):
    ...
    select = f"SELECT GEO.LATITUD, GEO.LONGITUD, GEO.DIRECCION, U.NOMBRE1, U.NOMBRE2, U.APELLIDO1, U.APELLIDO2 FROM SALUD.SF_GEOREFERENCIACION AS GEO LEFT JOIN SALUD.SF_PERSONAS AS U ON U.ID_USUARIO = GEO.ID_USUARIO WHERE DATE(GEO.FECMODI) = '2019-12-27' AND GEO.LATITUD  IS NOT NULL AND GEO.LONGITUD IS NOT NULL"
